...
def update_side(display_list,list_words):
    global dis
    dis.config(state='normal')
    dis.delete(0,END)
    for words in list_words: 
        dis.insert(END, words)
    del_msg(random_box)
    try:
        word = random.choice(list_words)
    except:
...
def text_entry():
    ...
    word_entry = Entry(frame, width = 13)
...
def message_log():
    ...
    message_banner.place(x=420, y=125 )
    my_entry = Entry(frame,width=13,background="grey10",fg="white")
    my_entry.pack()
...
def attempt_log():
    ...
    attempt_banner.place(x=185, y=175 )
    my_entry = Text(frame,height=8,width=32,background="grey10",fg="white")
    my_entry.pack()
    my_entry.config(state='disabled')
...
def check_box():
    ...
    def dis_other():
        for i in range(5):
            if green_val[i].get() == 1 and correct_index[i] == 0:
                yellow_buttons[i].config(state='disabled')
            elif green_val[i].get() == 0 and correct_index[i] == 0:
                if current_word[i]+str(i) not in letters_no_check:
                    yellow_buttons[i].config(state='normal')

        for i in range(5):
            if yellow_val[i].get() == 1 and correct_index[i] == 0:
                green_buttons[i].config(state='disabled')
            elif yellow_val[i].get() == 0 and correct_index[i] == 0:
                if current_word[i]+str(i) not in letters_no_check:
    ...
    for i in range(5):
        ...
        green_check.place(x=185, y=25 + (25 * i) )
    ...
    for i in range(5):
        ...
        yellow_check.place(x=300, y=25 + (25 * i) )
    ...
    button_done = Button(root, text ="Done",width=25,height=1, command = done_push,highlightbackground="grey10",fg="white")
    button_done.pack()
    button_done.place(x=185, y=150 )
...
def resuffle_button():
    ...
    button_random = Button(root, text ="Random Again",width=14,height=1,command=clicked)
    button_random.pack()
    button_random.place(x=425, y=233 )
...
def reset_button():
    ...
    button_play_again = Button(root, text ="Play Again",width=14,height=1,command=clicked)
    button_play_again.pack()
    button_play_again.place(x=425, y=260 )
...
def dark_button():
    ...
    def clicked():
        ...
        done_button.configure(highlightbackground="grey10",fg="white")
        for i in range(5):
            green_buttons[i].configure(background='gray20')
            yellow_buttons[i].configure(background='gray20')
        
    button_dark = Button(root, text ="Dark",width=7,height=1,command=clicked)
    button_dark.pack()
    button_dark.place(x=488, y=290 )
...
def light_button():
    ...
    def clicked():
        ...
        done_button.configure(highlightbackground="white smoke",fg="black")
        for i in range(5):
            green_buttons[i].configure(background='white')
            yellow_buttons[i].configure(background='white')
        
        
    button_light = Button(root, text ="Light",width=7,height=1,command=clicked)
    button_light.pack()
    button_light.place(x=425, y=290 )
...
def check_place():
    ...
    global correct_index
    for i in range(5):
        if current_word[i]+str(i) not in letters_no_check and correct_index[i] == 0:
            green_buttons[i].config(state='normal')
            yellow_buttons[i].config(state='normal')
